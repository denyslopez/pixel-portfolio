...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  return isCurrentLocale(lang) ? currentMetadata(lang, "axom") : {};
}

export default async function AxomPage({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params;
  if (!isCurrentLocale(lang)) notFound();
  return <DenysoftExperience locale={lang} page="axom" />;
}
